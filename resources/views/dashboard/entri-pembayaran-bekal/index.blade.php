...
@section('title', 'Pembayaran Bekal')

@section('breadcrumb')
    <li class="breadcrumb-item">Dashboard</li>
    <li class="breadcrumb-item active">Pembayaran Bekal</li>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Entri Pembayaran Bekal</div>

                    <form method="post" action="{{ url('/dashboard/pembayaran-bekal') }}">
                        @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="id_bekal" class="custom-select @error('id_bekal') is-invalid @enderror"
                                {{ count($bekals) == 0 ? 'disabled' : '' }}>
                                @if (count($bekals) == 0)
                                    <option>Pilihan tidak ada</option>
                                @else
                                    <option value="">Silahkan Pilih</option>
                                    @foreach ($bekals as $value)
                                        <option value="{{ $value->id }}">
                                            {{ 'Tahun ' . $value->tahun . ' - ' . 'Bulan ' . $value->bulan . ' - ' . 'Rp ' . $value->nominal }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                                            ...
                                            <div class="hide-menu">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                    <a class="dropdown-item"
                                                        href="{{ url('/dashboard/pembayaran-bekal/' . $value->id . '/edit') }}"><i
                                                            class="ti-pencil"></i> Edit </a>
                                                    <form method="post"
                                                        action="{{ url('/dashboard/pembayaran-bekal', $value->id) }}"
                                                        id="delete{{ $value->id }}">
                                                        @csrf
                                                        @method('delete')
...
@section('sweet')
    ...
    Swal.fire({
    title: 'PERINGATAN!',
    text: "Yakin ingin menghapus pembayaran bekal?",
    icon: 'warning',
