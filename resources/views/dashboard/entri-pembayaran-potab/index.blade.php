...
@section('title', 'Pembayaran Potab')

@section('breadcrumb')
    <li class="breadcrumb-item">Dashboard</li>
    <li class="breadcrumb-item active">Pembayaran Potab</li>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Entri Pembayaran Potab</div>

                    <form method="post" action="{{ url('/dashboard/pembayaran-potab') }}">
                        @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="id_potab" class="custom-select @error('id_potab') is-invalid @enderror"
                                {{ count($potabs) == 0 ? 'disabled' : '' }}>
                                @if (count($potabs) == 0)
                                    <option>Pilihan tidak ada</option>
                                @else
                                    <option value="">Silahkan Pilih</option>
                                    @foreach ($potabs as $value)
                                        <option value="{{ $value->id }}">
                                            {{ 'Tahun ' . $value->tahun . ' - ' . 'Bulan ' . $value->bulan . ' - ' . 'Rp ' . $value->nominal }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                                            ...
                                            <div class="hide-menu">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                    <a class="dropdown-item"
                                                        href="{{ url('/dashboard/pembayaran-potab/' . $value->id . '/edit') }}"><i
                                                            class="ti-pencil"></i> Edit </a>
                                                    <form method="post"
                                                        action="{{ url('/dashboard/pembayaran-potab', $value->id) }}"
                                                        id="delete{{ $value->id }}">
                                                        @csrf
                                                        @method('delete')
...
@section('sweet')
    ...
    Swal.fire({
    title: 'PERINGATAN!',
    text: "Yakin ingin menghapus pembayaran potab?",
    icon: 'warning',
