...
@section('title', 'Pembayaran Kegiatan')

@section('breadcrumb')
    <li class="breadcrumb-item">Dashboard</li>
    <li class="breadcrumb-item active">Pembayaran Kegiatan</li>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Entri Pembayaran Kegiatan</div>

                    <form method="post" action="{{ url('/dashboard/pembayaran-kegiatan') }}">
                        @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="id_kegiatan" class="custom-select @error('id_kegiatan') is-invalid @enderror"
                                {{ count($kegiatans) == 0 ? 'disabled' : '' }}>
                                ...
                                @else
                                    ...
                                    @foreach ($kegiatans as $value)
                                        <option value="{{ $value->id }}">
                                            {{ 'RP ' . $value->nominal . ' - ' . ' Kegiatan' . $value->keterangan }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">NOMINAL KEGIATAN</th>
                                            ...
                                            <div class="hide-menu">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                    <a class="dropdown-item"
                                                        href="{{ url('/dashboard/pembayaran-kegiatan/' . $value->id . '/edit') }}"><i
                                                            class="ti-pencil"></i> Edit </a>
                                                    <form method="post"
                                                        action="{{ url('/dashboard/pembayaran-kegiatan', $value->id) }}"
                                                        id="delete{{ $value->id }}">
                                                        @csrf
                                                        @method('delete')
...
@section('sweet')
    ...
    Swal.fire({
    title: 'PERINGATAN!',
    text: "Yakin ingin menghapus pembayaran Kegiatan?",
    icon: 'warning',
