...
@section('title', 'Pembayaran Ijazah')

@section('breadcrumb')
    <li class="breadcrumb-item">Dashboard</li>
    <li class="breadcrumb-item active">Pembayaran Ijazah</li>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Entri Pembayaran Ijazah</div>

                    <form method="post" action="{{ url('/dashboard/pembayaran-ijazah') }}">
                        @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="id_ijazah" class="custom-select @error('id_ijazah') is-invalid @enderror"
                                {{ count($ijazahs) == 0 ? 'disabled' : '' }}>
                                @if (count($ijazahs) == 0)
                                    <option>Pilihan tidak ada</option>
                                @else
                                    <option value="" hidden>Silahkan Pilih</option>
                                    @foreach ($ijazahs as $value)
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">NOMINAL IJAZAH</th>
                                            ...
                                            <div class="hide-menu">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                    <a class="dropdown-item"
                                                        href="{{ url('/dashboard/pembayaran-ijazah/' . $value->id . '/edit') }}"><i
                                                            class="ti-pencil"></i> Edit </a>
                                                    <form method="post"
                                                        action="{{ url('/dashboard/pembayaran-ijazah', $value->id) }}"
                                                        id="delete{{ $value->id }}">
                                                        @csrf
                                                        @method('delete')
...
@section('sweet')
    ...
    Swal.fire({
    title: 'PERINGATAN!',
    text: "Yakin ingin menghapus pembayaran Ijazah?",
    icon: 'warning',
