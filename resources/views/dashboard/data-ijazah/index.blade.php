...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <div class="card-title">{{ __('Tambah Ijazah') }}</div>

                    <form method="post" action="{{ url('/dashboard/data-ijazah') }}">
                        @csrf
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Data Ijazah</div>
                                            ...
                                            <div class="hide-menu">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                    <a class="dropdown-item"
                                                        href="{{ url('dashboard/data-ijazah/' . $value->id . '/edit') }}"><i
                                                            class="ti-pencil"></i> Edit </a>
                                                    <form method="post"
                                                        action="{{ url('dashboard/data-ijazah', $value->id) }}"
                                                        id="delete{{ $value->id }}">
                                                        @csrf
                                                        @method('delete')
                    ...
                    <!-- Pagination -->
                    @if ($ijazah->lastPage() != 1)
                        <div class="btn-group float-right">
                            <a href="{{ $app->previousPageUrl() }}" class="btn btn-success">
                                <i class="mdi mdi-chevron-left"></i>
                            </a>
                            @for ($i = 1; $i <= $ijazah->lastPage(); $i++)
                                <a class="btn btn-success {{ $i == $ijazah->currentPage() ? 'active' : '' }}"
                                    href="{{ $ijazah->url($i) }}">{{ $i }}</a>
                            @endfor
                            <a href="{{ $ijazah->nextPageUrl() }}" class="btn btn-success">
...
@section('sweet')
    ...
    Swal.fire({
    title: 'PERINGATAN!',
    text: "Yakin ingin menghapus data Ijazah?",
    icon: 'warning',
